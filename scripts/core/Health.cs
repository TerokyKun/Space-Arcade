...
public partial class Health : Node
{
    ...
    // Возврат к жизни (Second Wind / доп. жизнь): сбрасывает флаг смерти
    // и восстанавливает долю здоровья от max (percent в долях 0..1).
    public void Revive(float percent)
    {
        ...
        _isDead = false;
        CurrentHP = Mathf.Clamp(MaxHP * Mathf.Clamp(percent, 0f, 1f), 1f, MaxHP);
        EmitSignal(SignalName.HealthChanged, CurrentHP, MaxHP);
    }
}