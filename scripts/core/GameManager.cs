...
public partial class GameManager : Node
{
    ...
    // Game Over: блокирует начисление очков/опыта после смерти игрока,
    // замораживает спавн и активность мира (за исключением меню).
    public static bool IsGameOver { get; private set; }
    private const string GameOverLock = "game_over";
    ...
    {
        IsGameOver = true;
        AddPauseLock(GameOverLock);
        GD.Print("🏁 GAME OVER — начисление очков заблокировано");
    }

    public void NotifyGameRestart()
    {
        IsGameOver = false;
        RemovePauseLock(GameOverLock);
    }
}